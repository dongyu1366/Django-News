...
class News:
    ...
    def get_news_list(cls):
        for category in NewsApi.CATEGORY:
            ...
            for news in news_list:
                token = news['Id']
                cur.execute(f'SELECT title FROM news_list WHERE token={token}')
                if not cur.fetchone():
                    DbHandler.insert_list_to_db(news=news)
            print(f'News List: Get {number} news from {category}')
    ...
    def _fetch_all_news_content(cls, category):
        cur.execute('SELECT url FROM news_list WHERE category=?', (category,))
    ...
    def get_news(cls):
        ...
        for category in category_list:
            ...
            for news in news_contents:
                token = news['token']
                cur.execute(f'SELECT title FROM news WHERE token={token}')
                if not cur.fetchone():
